...
mod test {
    ...
    fn ahp() -> Result<(), Error> {
        ...
        let (ps, first_oracles) = AHPForPLONK::prover_first_round(ps, &cs, rng)?;
        let (vs, first_msg) = AHPForPLONK::verifier_first_round(vs, rng)?;

        let (ps, second_oracles) =
            AHPForPLONK::prover_second_round(ps, &first_msg, &ks, rng)?;
        ...
        let is_equal =
            AHPForPLONK::verifier_equality_check(&vs, &evaluations, cs.public_inputs())?;
